...
const CategoryTable = ({ categories, onAdd, onEdit, onDelete }) => {
  ...
  const filterCategories = (categoriesData, searchTerm) => {
        ...
        <table className="w-full mb-20">
          <thead>
            <tr className="bg-gray-200">
              {["categoryId", "image", "name", "actions"].map(
                (header, index, array) => (
                  <th
                    key={header}
                    className={`p-3 text-center gap-4  cursor-poiner ${
          ...
          <tbody>
            {currentData.length === 0 ? (
              ...
              currentData.map((category) => (
                ...
                >
                  <td className="p-3 text-center">{category.categoryId}</td>
